...
| Tests used for selection | Regression-test selection and reproduction-test generation, followed by execution | Recorded regression exclusions and reproduction samples; baseline-eligible tests vote by exact source to select one for candidate checks | [Regression selection](regression-selection.md), [reproduction tests](reproduction-tests.md) and [validation note](public-validation.md). Commands can declare JUnit XML or CTRF JSON reports so selection counts individual failing tests ([per-test results](public-validation.md#per-test-results-from-reports)). Test-level regression selection, automatic discovery, live generation and upstream normalization/marker-protocol parity remain unfinished. |
