...
| Tests used for selection | Regression-test selection and reproduction-test generation, followed by execution | Fixed schedules or passing-baseline inventory plus recorded regression exclusions; reproduction commands remain caller-supplied | [Regression selection](regression-selection.md) and [validation note](public-validation.md). Automatic discovery, framework reports, live selection and reproduction-test generation remain unfinished. |
